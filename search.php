...
if(isset($_POST['search'])){
    $s=$_POST['search'];
    $search="where caname like '%$s%' or cavenue like '%$s%' or cacategory like '%$s%' or cadate like '%$s%'";
}else{
  echo "<script>window.location.assign('index.php');</script>";
}
...
<html>
<head>
	<title>Levent - Search for "<?php echo $s; ?>"</title>
...
<p style="font-size: 200%;font-weight: 450;text-align: center;">Search for "<?php echo $s; ?>"</p>
<hr>
...
$record=0;
$astatus='true';
$sql = "SELECT caid,causer,caname,cadate,cadate2,atime,atime2,cavenue,cacategory,cadescription,cacommittee,catime,status FROM event ".$search." AND status='$astatus' ORDER BY caid DESC";
